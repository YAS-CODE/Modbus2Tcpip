...
mDNSexport void mDNSPosixProcessFDSet(mDNS *const m, fd_set *readfds,time_t *current)
	{
...
mStatus mDNSPosixRunEventLoopOnce(mDNS *m, const struct timeval *pTimeout, 
	...
	if (numReady > 0)
		...
		(void) mDNSPosixProcessFDSet(m, &listenFDs,NULL);	// call this first to process wire data for clients
