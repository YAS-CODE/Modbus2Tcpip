...
int main(int argc, char **argv)
{
    ...
    time_t last_update_t,current_utc;

    time(&current_utc);
    last_update_t=current_utc;
...
//                timeout.tv_usec = 0;
		...
		if (result < 0)
			...
			mDNSPosixProcessFDSet(&mDNSStorage, &readfds,&current_utc);
//			last_update_t=current_utc;
			sec = difftime(last_update_t,current_utc);
			if(sec>60){
				printf("yas %d: should exit here noew :) func=%s, file=%s\n",__LINE__,__FUNCTION__,__FILE__);
				gStopNow = mDNStrue;
			}
