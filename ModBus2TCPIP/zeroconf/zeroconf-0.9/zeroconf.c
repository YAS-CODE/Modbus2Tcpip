...
int main(int argc, char **argv)
{
 ...
 /* display result */
 printf("%s-%s\n", inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr),inet_ntoa(((struct sockaddr_in *)&ifr.ifr_broadaddr)->sin_addr));
 memset(ip_address,0,30);
 strcpy(ip_address,inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr));
 len=strlen(ip_address);
 index=len;
 while(ip_address[index]!='.')
        index--;
 strcpy(ip_last,ip_address+index+1);
 printf("last_ip=%s\n",ip_last);
 if(strcmp(ip_last,"0")==0)
        printf("dhcp not found!\n");
 else{
        printf("dhcp is already working....!\n");
	exit(0);
 }
...
//  if (!nofork)
  ...
  while (keep_running) {
    ...
    if (!intf.up) {
      fprintf(stderr,"yas %d: state=%d func=%s, file=%s\n",__LINE__,state,__FUNCTION__,__FILE__);
